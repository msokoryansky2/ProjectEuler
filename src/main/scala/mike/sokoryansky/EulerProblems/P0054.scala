...
import mike.sokoryansky.MathUtils.{Hand, String, PolygonalNumber3}
...
class P0054 extends  EulerProblem {
  def run: String = Source.fromResource("p054_poker.txt").getLines.map(Hand.twoHands).count(h => h._1 > h._2).toString
}

object P0054 extends App {
  (new P0054).printAnswer()
}
